...
const calculateStats = (questions) => {
  ...
  const score = questions.reduce((acc, question) => {
    return acc + (question?.userAnswer !== undefined && question?.userAnswer === question?.answer ? 1 : 0);
  }, 0);
...
const filterQuestions = (questions, selectedFilter) => {
  ...
  if (selectedFilter === 'correct') {
    return questions.filter(q => typeof q.userAnswer === 'number' && q.userAnswer === q.answer);
  }
...
const CollapsibleQuestionCard = React.memo(
  ({ question, questionIndex, handleOptionSelect }) => {
    ...
    const toggleExpanded = () => {
      ...
      animatedHeight.value = withTiming(isExpanded ? 0 : 1, { duration: ANIMATION_DURATION });
      chevronRotation.value = withTiming(isExpanded ? 0 : 180, { duration: ANIMATION_DURATION });
    };
...
const HeaderCard = ({ topic, prompt }) => (
  <Card className="mb-6">
    <CardHeader>
      <View className="flex-row items-center justify-between">
        <View className="flex-1">
          <CardTitle className="text-2xl font-bold text-foreground mb-1">{topic}</CardTitle>
          <CardDescription className="text-base text-muted-foreground">{prompt}</CardDescription>
        </View>
      </View>
    </CardHeader>
  </Card>
...
const ActionButtons = ({ onProgressPress, onGenerateQuestions, generateMoreQuestionsStatus, questionBookId, dispatch }) => (
        ...
        {filteredQuestions?.length === 0 ? (
          ...
          showAccordionView ? (
            <FlatList
              ...
              ListHeaderComponent={
                <HeaderCard topic={currentQuestionBook?.topic} prompt={currentQuestionBook?.prompt} />
              }
              ListFooterComponent={
                <ActionButtons
                  onProgressPress={() => setIsDrawerVisible(true)}
                  onGenerateQuestions={() => dispatch(generateMoreQuestions(questionBookId))}
